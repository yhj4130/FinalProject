$(document).ready(function()
{
    ...
    $('input:checkbox[name=loc]').change(function()
    {
		if($(this).is(':checked'))
		{
			$(this).parent('label.b1').css('color', '#21bf34;');
		}
		else
		{
  ...
  /* 	alert("하핫");*/
    ...
    $('.checkAll').change(function()
    {
    	if($('.checkAll').prop('checked'))
    	{
    		...
    		$(this).parent('label').css('color', '#21bf34;');
    	}
    	else
    	{
    		$('.age').prop('checked', false);
    ...
    $('input:checkbox[name=age]').change(function(){ 	
    	var a = $("input:checkbox[name=age]");
    	if(a.length == a.filter(":checked").length){
    		/*alert('모두 체크되었습니다');*/       		
    		$("input:checkbox[name=age]").prop('checked', false) ;
    		$("input:checkbox[name=age]").parent('label').css('color', '#848484');
    		$('.checkAll').prop('checked',true);
    		$('.checkAll').parent('label').css('color', '#21bf34');
    		/*alert("전부체크");  */  		
    	}
