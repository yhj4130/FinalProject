...
$(document)
		.ready(
				function() {
					...
					$('input:checkbox[name=loc]').change(
							function() {
								if ($(this).is(':checked')) {
					...
					$('input:radio[name=gender]').on(
							...
							function() {
								if ($(this).is(':checked')) {
									$('input:radio[name=gender]').parent(
											'label').css('color', '#848484');
									$(this).parent('label').css('color',
											'#21bf34');
								}
					...
					$('#upload').on('click', function() {
						...
						if ($('#subject').val() == ""||$('#subject').val().length>30) {
							alert("제목을 30자이내로 입력하세요.");
							$("#subject").focus();
						} 
						else if (($("#loc:checked").length) == 0) {
							alert("카테고리를 선택해주세요");
							$("#loc").focus();
						} 
						else if ($("input:radio[name='age']").is(":checked")!=true) {
							alert("연령을 선택해주세요");
							...
							if (count==1 && ($("div.sec.insert").eq(0).find("input:text[name='addr']").val()=="")) {
								
								alert("여행지는 최소 한개는 작성하셔야 합니다. 주소검색을 실행하세요");
								$("div.sec.insert").eq(0).find("input:text[name='addr']").focus();
							
							} else if(count>1){
								
								for (var i = 0; i < count; i++) {
									
									if ($("div.sec.insert").eq(i).find("input:text[name='addr']").val()=="") {
										
										alert("주소검색을 실행하세요.");
										$("div.sec.insert").eq(i).find("input:text[name='addr']").focus();
										return;							
									}
								}
							} else {
										...
										$('.body.section6.margin20')
												.append(
														"<div class='sec insert'>"
																...
																+ "<input type='text' name='addr' placeholder='검색을 눌러 검색해주세요'"
																+ "id='addr"
																+ num
					...
					// 여행지삭제 클릭시 이벤트
					$( document ).on( "click",".fas.fa-trash", function(e) {
						/* $(event.target).closest(".sec.insert").children().remove(); */
							...
							if (count<=1) {
								...
								$(e.target).closest(".sec.insert").remove();
								count--;				
							
								for (var i = 0; i < count; i++) {
									$("div.sec.insert").eq(i).find('.step').html("step "+(i+1)+".<br><i class='fas fa-trash'></i>");
									$("div.sec.insert").eq(i).find('.input-group-prepend').children(":text").attr('id','addr'+i);
									$("div.sec.insert").eq(i).find('.input-group-prepend').children(":button").attr('id','findaddr'+i);
									$("div.sec.insert").eq(i).find('.input-group-prepend').children("input[name=lat]").attr('id','lat'+i);
									$("div.sec.insert").eq(i).find('.input-group-prepend').children("input[name=lon]").attr('id','lon'+i);
								}
								--num;
								$("div.sec.insert").eq(0).find('.subtitle').text("여행지 추가");
							}
					...
					// 여행지 세부 사항 클릭시 이벤트
					$(document).on('click', '.traffic', function(e) {
						if ( $(e.target).closest(".insertform").find('.car.arrayflex').length > 0 ) {
							
							alert("이미 추가하셨습니다. 추가한 목록에 작성해 주시기 바랍니다.");
							
						} else {
							
							$(e.target).closest(".insertform").children('.insert-cons.details').append("" +
					...
					$(document).on('click', '.pay', function(e) {
						if ( $(e.target).closest(".insertform").find('.won-sign.arrayflex').length > 0 ) {
					...
					$(document).on('click', '.time', function(e) {
						if ( $(e.target).closest(".insertform").find('.stopwatch.arrayflex').length > 0 ) {
					...
					$(document).on('click', '.else', function(e) {
						if ( $(e.target).closest(".insertform").find('.calendar.arrayflex').length > 0 ) {
...
function openChild(val) {
	...
	var addr = $(val).siblings("input[name=addr]").attr('id');
	// window.open("open할 window", "자식창 이름", "팝업창 옵션");
	var windowW = 700; // 창의 가로 길이
	var windowH = 700; // 창의 세로 길이
	var left = Math.ceil((window.screen.width - windowW) / 2);
	var top = Math.ceil((window.screen.height - windowH) / 2);
	window.open("addrSearchForm.do?lonName=" + lonName+"&latName="+latName+"&addr="+addr, "childForm",
			"scrollbars = no,l top=" + top + ", left=" + left + ", height="
